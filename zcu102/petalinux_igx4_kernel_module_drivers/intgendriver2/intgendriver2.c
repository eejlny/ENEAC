/*
     ...
     Date: 05 Feb 2019
     Description: Kernel module driver for interrupt generator 2
*/
/*  intgendriver2.c - The simplest kernel module.
...
MODULE_AUTHOR
    ("Dr Kris Nikov");
MODULE_DESCRIPTION
    ("intgendriver2 - Interrupt generator driver module.");

#define DRIVER_NAME "intgendriver2"
...
uint intgendriver2_read(struct file *filp, char *buf, size_t count, loff_t *f_pos);
ssize_t intgendriver2_write(struct file *filp, char *buf, size_t count, loff_t *f_pos);
int intgendriver2_open(struct inode *inode, struct file *filp);
int intgendriver2_release(struct inode *inode, struct file *filp);
long intgendriver2_ioctl(struct file *file,	/* ditto */
                         unsigned int ioctl_num,	/* number and param for ioctl */
                         unsigned long ioctl_param); //data from userspace

struct intgendriver2_local {
	dev_t devno;
	int irq;
...
static irqreturn_t my_mb_task_done (int irq, void *lp) {
     ...
     intr0 = (int*)((struct intgendriver2_local*) lp)->base_addr;
     if (DBG) {
...
struct file_operations intgendriver2_fops = {
     .owner = THIS_MODULE,
     .read = (void*)intgendriver2_read,
     .write = (void*)intgendriver2_write,
     .open = intgendriver2_open,
	.unlocked_ioctl = intgendriver2_ioctl,
     .release = intgendriver2_release
};

static int intgendriver2_probe(struct platform_device *pdev) {
	struct resource *r_irq; /* Interrupt resources */
	struct resource *r_mem; /* IO mem resources */
	struct device *dev = &pdev->dev;
	struct intgendriver2_local *lp = NULL;
	int *intr2;

	int status;
	int rc = 0;
	
	dev_info(dev, "intgendriver2 Device Tree Probing\n");
	...
	lp = (struct intgendriver2_local *) kmalloc(sizeof(struct intgendriver2_local), GFP_KERNEL);
	if (!lp) {
		dev_err(dev, "Cound not allocate intgendriver2 device\n");
		return -ENOMEM;
	}
	...
	if (!lp->base_addr) {
		dev_err(dev, "intgendriver2: Could not allocate iomem\n");
		rc = -EIO;
		goto error2;
	}
	...
	status= register_chrdev(MAJOR_NUM, DRIVER_NAME, &intgendriver2_fops);
	if(status <0) {
          printk("register_Drv\n");
     }
	
	cdev_init(&lp->cdev, &intgendriver2_fops);
	lp->cdev.owner = THIS_MODULE;
	lp->cdev.ops = &intgendriver2_fops;
	cdev_add(&lp->cdev, lp->devno,1);
	if(status < 0) {
		printk("Problem in registering Major\n");
     }
	
	dev_info(dev,"intgendriver2 at 0x%08x mapped to 0x%08x, irq=%d\n",(unsigned int __force)lp->mem_start,(unsigned int __force)lp->base_addr,lp->irq);
	printk(KERN_DEBUG "Interrupt2 ready!\n");
...
static int intgendriver2_remove(struct platform_device *pdev) {
	struct device *dev = &pdev->dev;
	struct intgendriver2_local *lp = dev_get_drvdata(dev);
	cdev_del(&lp->cdev);
...
uint intgendriver2_read(struct file *filp, char __user *buf, size_t count, loff_t *f_pos) {
	struct intgendriver2_local *dev = filp->private_data;
	...
	switch (ioctl_num) {
		case IOCTL_WAIT_INTERRUPT:
               if (DBG) {
                    ...
                    printk(KERN_DEBUG "intgendriver2, workload size set at %d\n", args.spmm_workload);
               }
			if (TIMEOUT) {
			...
			if (DBG) {
                    ...
                    printk(KERN_DEBUG "Reg0 set at %d\n", *intr0);
                    printk(KERN_DEBUG "WaitQueue flag set at %d\n", flag);
                    printk(KERN_DEBUG "intgendriver2, workload size was %d\n", args.spmm_workload);
               }
			flag = 0;
...
int intgendriver2_open(struct inode *inode, struct file *filp) {
	struct intgendriver2_local *dev;
	dev = container_of(inode->i_cdev, struct intgendriver2_local, cdev);
     filp->private_data = dev;       /* For use elsewhere */
     printk(KERN_DEBUG "intgendriver2 open and ready\n");
...
static struct of_device_id intgendriver2_of_match[] = {
	{ .compatible = "kris,intgendriver2-1.01.a", },
	{ /* end of list */ },
};
MODULE_DEVICE_TABLE(of, intgendriver2_of_match);

static struct platform_driver intgendriver2_driver = {
	.driver = {
		.name = DRIVER_NAME,
		.owner = THIS_MODULE,
		.of_match_table = intgendriver2_of_match,
	},
	.probe = intgendriver2_probe,
	.remove = intgendriver2_remove,
};

static int __init intgendriver2_init(void) {
	printk(KERN_DEBUG "Hello interrupts!\n");
	return platform_driver_register(&intgendriver2_driver);
}


static void __exit intgendriver2_exit(void) {
	platform_driver_unregister(&intgendriver2_driver);
	printk(KERN_DEBUG "Goodbye interrupts!\n");
}

module_init(intgendriver2_init);
module_exit(intgendriver2_exit);
