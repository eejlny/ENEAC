/*
     ...
     Date: 05 Feb 2019
     Description: Kernel module driver for interrupt generator 1
*/
/*  intgendriver1.c - The simplest kernel module.
...
MODULE_AUTHOR
    ("Dr Kris Nikov");
MODULE_DESCRIPTION
    ("intgendriver1 - Interrupt generator driver module.");

#define DRIVER_NAME "intgendriver1"
...
uint intgendriver1_read(struct file *filp, char *buf, size_t count, loff_t *f_pos);
ssize_t intgendriver1_write(struct file *filp, char *buf, size_t count, loff_t *f_pos);
int intgendriver1_open(struct inode *inode, struct file *filp);
int intgendriver1_release(struct inode *inode, struct file *filp);
long intgendriver1_ioctl(struct file *file,	/* ditto */
                         unsigned int ioctl_num,	/* number and param for ioctl */
                         unsigned long ioctl_param); //data from userspace

struct intgendriver1_local {
	dev_t devno;
	int irq;
...
static irqreturn_t my_mb_task_done(int irq, void *lp) {
     ...
     intr0 = (int*)((struct intgendriver1_local*) lp)->base_addr;
     if (DBG) {
...
struct file_operations intgendriver1_fops = {
     .owner = THIS_MODULE,
     .read = (void*)intgendriver1_read,
     .write = (void*)intgendriver1_write,
     .open = intgendriver1_open,
	.unlocked_ioctl = intgendriver1_ioctl,
     .release = intgendriver1_release
};

static int intgendriver1_probe(struct platform_device *pdev) {
	struct resource *r_irq; /* Interrupt resources */
	struct resource *r_mem; /* IO mem resources */
	struct device *dev = &pdev->dev;
	struct intgendriver1_local *lp = NULL;
	int *intr2;

	int status;
	int rc = 0;
	
	dev_info(dev, "intgendriver1 Device Tree Probing\n");
	...
	lp = (struct intgendriver1_local *) kmalloc(sizeof(struct intgendriver1_local), GFP_KERNEL);
	if (!lp) {
		dev_err(dev, "Cound not allocate intgendriver1 device\n");
		return -ENOMEM;
	}
	...
	if (!lp->base_addr) {
		dev_err(dev, "intgendriver1: Could not allocate iomem\n");
		rc = -EIO;
		goto error2;
	}
	...
	status= register_chrdev(MAJOR_NUM, DRIVER_NAME, &intgendriver1_fops);
	if(status <0) {
          printk("register_Drv\n");
     }
	
	cdev_init(&lp->cdev, &intgendriver1_fops);
	lp->cdev.owner = THIS_MODULE;
	lp->cdev.ops = &intgendriver1_fops;
	cdev_add(&lp->cdev, lp->devno,1);
	if(status < 0) {
		printk("Problem in registering Major\n");
     }
	
	dev_info(dev,"intgendriver1 at 0x%08x mapped to 0x%08x, irq=%d\n",(unsigned int __force)lp->mem_start,(unsigned int __force)lp->base_addr,lp->irq);
	printk(KERN_DEBUG "Interrupt1 ready!\n");
...
static int intgendriver1_remove(struct platform_device *pdev) {
	struct device *dev = &pdev->dev;
	struct intgendriver1_local *lp = dev_get_drvdata(dev);
	cdev_del(&lp->cdev);
...
uint intgendriver1_read(struct file *filp, char __user *buf, size_t count, loff_t *f_pos) {
	struct intgendriver1_local *dev = filp->private_data;
	...
	switch (ioctl_num) {
		case IOCTL_WAIT_INTERRUPT:
               if (DBG) {
                    ...
                    printk(KERN_DEBUG "intgendriver1, workload size set at %d\n", args.spmm_workload);
               }
			if (TIMEOUT) {
			...
			if (DBG) {
                    ...
                    printk(KERN_DEBUG "Reg0 set at %d\n", *intr0);
                    printk(KERN_DEBUG "WaitQueue flag set at %d\n", flag);
                    printk(KERN_DEBUG "intgendriver1, workload size was %d\n", args.spmm_workload);
               }
			flag = 0;
...
int intgendriver1_open(struct inode *inode, struct file *filp) {
	struct intgendriver1_local *dev;
	dev = container_of(inode->i_cdev, struct intgendriver1_local, cdev);
     filp->private_data = dev;       /* For use elsewhere */
     printk(KERN_DEBUG "intgendriver1 open and ready\n");
...
static struct of_device_id intgendriver1_of_match[] = {
	{ .compatible = "kris,intgendriver1-1.01.a", },
	{ /* end of list */ },
};
MODULE_DEVICE_TABLE(of, intgendriver1_of_match);

static struct platform_driver intgendriver1_driver = {
	.driver = {
		.name = DRIVER_NAME,
		.owner = THIS_MODULE,
		.of_match_table = intgendriver1_of_match,
	},
	.probe = intgendriver1_probe,
	.remove = intgendriver1_remove,
};

static int __init intgendriver1_init(void) {
	printk(KERN_DEBUG "Hello interrupts!\n");
	return platform_driver_register(&intgendriver1_driver);
}


static void __exit intgendriver1_exit(void) {
	platform_driver_unregister(&intgendriver1_driver);
	printk(KERN_DEBUG "Goodbye interrupts!\n");
}

module_init(intgendriver1_init);
module_exit(intgendriver1_exit);
