/*
     ...
     Date: 05 Feb 2019
     Description: Kernel module driver for interrupt generator 3
*/
/*  intgendriver3.c - The simplest kernel module.
...
MODULE_AUTHOR
    ("Dr Kris Nikov");
MODULE_DESCRIPTION
    ("intgendriver3 - Interrupt generator driver module.");

#define DRIVER_NAME "intgendriver3"
...
uint intgendriver3_read(struct file *filp, char *buf, size_t count, loff_t *f_pos);
ssize_t intgendriver3_write(struct file *filp, char *buf, size_t count, loff_t *f_pos);
int intgendriver3_open(struct inode *inode, struct file *filp);
int intgendriver3_release(struct inode *inode, struct file *filp);
long intgendriver3_ioctl(struct file *file,	/* ditto */
                         unsigned int ioctl_num,	/* number and param for ioctl */
                         unsigned long ioctl_param); //data from userspace

struct intgendriver3_local {
	dev_t devno;
	int irq;
...
static irqreturn_t my_mb_task_done (int irq, void *lp) {
     ...
     intr0 = (int*)((struct intgendriver3_local*) lp)->base_addr;
     if (DBG) {
...
struct file_operations intgendriver3_fops = {
     .owner = THIS_MODULE,
     .read = (void*)intgendriver3_read,
     .write = (void*)intgendriver3_write,
     .open = intgendriver3_open,
	.unlocked_ioctl = intgendriver3_ioctl,
     .release = intgendriver3_release
};

static int intgendriver3_probe(struct platform_device *pdev) {
	struct resource *r_irq; /* Interrupt resources */
	struct resource *r_mem; /* IO mem resources */
	struct device *dev = &pdev->dev;
	struct intgendriver3_local *lp = NULL;
	int *intr2;

	int status;
	int rc = 0;
	
	dev_info(dev, "intgendriver3 Device Tree Probing\n");
	...
	lp = (struct intgendriver3_local *) kmalloc(sizeof(struct intgendriver3_local), GFP_KERNEL);
	if (!lp) {
		dev_err(dev, "Cound not allocate intgendriver3 device\n");
		return -ENOMEM;
	}
	...
	if (!lp->base_addr) {
		dev_err(dev, "intgendriver3: Could not allocate iomem\n");
		rc = -EIO;
		goto error2;
	}
	...
	status= register_chrdev(MAJOR_NUM, DRIVER_NAME, &intgendriver3_fops);
	if(status <0) {
          printk("register_Drv\n");
     }
	
	cdev_init(&lp->cdev, &intgendriver3_fops);
	lp->cdev.owner = THIS_MODULE;
	lp->cdev.ops = &intgendriver3_fops;
	cdev_add(&lp->cdev, lp->devno,1);
	if(status < 0) {
		printk("Problem in registering Major\n");
     }
	
	dev_info(dev,"intgendriver3 at 0x%08x mapped to 0x%08x, irq=%d\n",(unsigned int __force)lp->mem_start,(unsigned int __force)lp->base_addr,lp->irq);
	printk(KERN_DEBUG "Interrupt3 ready!\n");
...
static int intgendriver3_remove(struct platform_device *pdev) {
	struct device *dev = &pdev->dev;
	struct intgendriver3_local *lp = dev_get_drvdata(dev);
	cdev_del(&lp->cdev);
...
uint intgendriver3_read(struct file *filp, char __user *buf, size_t count, loff_t *f_pos) {
	struct intgendriver3_local *dev = filp->private_data;
	...
	switch (ioctl_num) {
		case IOCTL_WAIT_INTERRUPT:
               if (DBG) {
                    ...
                    printk(KERN_DEBUG "intgendriver3, workload size set at %d\n", args.spmm_workload);
               }
			if (TIMEOUT) {
			...
			if (DBG) {
                    ...
                    printk(KERN_DEBUG "Reg0 set at %d\n", *intr0);
                    printk(KERN_DEBUG "WaitQueue flag set at %d\n", flag);
                    printk(KERN_DEBUG "intgendriver3, workload size was %d\n", args.spmm_workload);                    
               }
			flag = 0;
...
int intgendriver3_open(struct inode *inode, struct file *filp) {
	struct intgendriver3_local *dev;
	dev = container_of(inode->i_cdev, struct intgendriver3_local, cdev);
     filp->private_data = dev;       /* For use elsewhere */
     printk(KERN_DEBUG "intgendriver3 open and ready\n");
...
static struct of_device_id intgendriver3_of_match[] = {
	{ .compatible = "kris,intgendriver3-1.01.a", },
	{ /* end of list */ },
};
MODULE_DEVICE_TABLE(of, intgendriver3_of_match);

static struct platform_driver intgendriver3_driver = {
	.driver = {
		.name = DRIVER_NAME,
		.owner = THIS_MODULE,
		.of_match_table = intgendriver3_of_match,
	},
	.probe = intgendriver3_probe,
	.remove = intgendriver3_remove,
};

static int __init intgendriver3_init(void) {
	printk(KERN_DEBUG "Hello interrupts!\n");
	return platform_driver_register(&intgendriver3_driver);
}


static void __exit intgendriver3_exit(void) {
	platform_driver_unregister(&intgendriver3_driver);
	printk(KERN_DEBUG "Goodbye interrupts!\n");
}

module_init(intgendriver3_init);
module_exit(intgendriver3_exit);
