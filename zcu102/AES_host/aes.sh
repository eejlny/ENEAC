#!/bin/bash
#AES control script for easier data collection
#KrNikov 2019
...
while getopts ":n:f:c:p:l:k:d:s:it:h" opt;
do
    case $opt in
          h)
               echo "Available flags and options:"
               echo "-n [INT] -> Select number of executions."
               echo "-f [FILE] -> Input file to encrypt."
               ...
               echo "-t [FILE] -> Select the save file for the encrypted output."
               echo "Mandatory options are: -n [INT] -f [FILE] -c [0-4] -p [0-4] -l [0-2] -k [INT] -d [1-2]"
               exit 0 
               ;;
...
if [[ -z $in_file ]]; then
     ...
     if ! [[ -e "$in_file" ]]; then
          echo "-f $in_file does not exist. Please enter a valid input file to encrypt!" >&2
          exit 1
     fi
fi
...
do
     if [[ -z $out_file ]]; then
          aes_output+=$(./AES $in_file $contin $cpuc $hpacc $hpcacc $chunk $ioctl 2> /dev/null)
          #echo "./AES $in_file $cpuc $hpacc $hpcacc $chunk $ioctl 2> /dev/null"
          #exit
     else
          aes_output+=$(./AES $in_file $cpuc $hpacc $hpcacc $chunk $ioctl $out_file 2> /dev/null)
     fi
done

#Extract relevant informaton from aes output. These will depend on output mode.
#Exec. Time
IFS=";" read -a exec_time <<< $(echo -e "$aes_output" | awk -v SEP=' ' 'BEGIN{FS=SEP}{if ($1=="CLOCK_REALTIME"){ print $3 * 1000 }}' | tr "\n" ";" | head -c -1)
#Workload on CPU
IFS=";" read -a cpu_work <<< $(echo -e "$aes_output" | awk -v SEP=' ' 'BEGIN{FS=SEP}{if ($1=="Total" && $2=="workload" && $4=="CPU:" ){ print $5 }}' | tr "\n" ";" | head -c -1)
#Workload on FPGA
IFS=";" read -a fpga_work <<< $(echo -e "$aes_output" | awk -v SEP=' ' 'BEGIN{FS=SEP}{if ($1=="Total" && $2=="workload" && $4=="FPGA:" ){ print $5 }}' | tr "\n" ";" | head -c -1)
#CPU Power
IFS=";" read -a cpu_power <<< $(echo -e "$aes_output" | awk -v SEP=' ' 'BEGIN{FS=SEP}{if ($1=="Processing" && $2=="System" && $4=="POWER" ){ print $6 }}' | tr "\n" ";" | head -c -1)
#FPGA Power    
IFS=";" read -a fpga_power <<< $(echo -e "$aes_output" | awk -v SEP=' ' 'BEGIN{FS=SEP}{if ($1=="Programmable" && $2=="Logic" && $4=="POWER" ){ print $6 }}' | tr "\n" ";" | head -c -1)
#CPU Energy
IFS=";" read -a cpu_energy <<< $(echo -e "$aes_output" | awk -v SEP=' ' 'BEGIN{FS=SEP}{if ($1=="Processing" && $2=="System" && $4=="ENERGY" ){ print $6 }}' | tr "\n" ";" | head -c -1)
#FPGA Energy
IFS=";" read -a fpga_energy <<< $(echo -e "$aes_output" | awk -v SEP=' ' 'BEGIN{FS=SEP}{if ($1=="Programmable" && $2=="Logic" && $4=="ENERGY" ){ print $6 }}' | tr "\n" ";" | head -c -1)
#Chunk size for MultiHap scheduler
[[ $sched == 2 ]] && IFS=";" read -a chunk_arr <<< $(echo -e "$aes_output" | awk -v SEP=' ' 'BEGIN{FS=SEP}{if ($1=="Final" && $2=="FPGA" && $3=="Chunk:" ){ print $4 }}' | tr "\n" ";" | head -c -1)
