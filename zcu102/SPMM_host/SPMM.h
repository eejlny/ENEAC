...
#define x_width 100 //if you change this then both CPUs and FPGA will process this number of x vectors
//#define USE_HP 1    // define this to use the HP ports non_cacheable (otherwise HPC ports cacheable)
...
void spmvCpu(const floatType *val, const int *cols, const int *rowDelimiters, 
	     const floatType *vec, floatType *out, int begin, int end) 
{
...
template <typename floatType>
void spmvCpu_vilches(const floatType *val, const int *cols, const int *rowDelimiters, 
	     const floatType *vec, floatType *out,  int begin, int end){
                ...
                int **rowDelimiters_ptr, int *n, int *size, int *columns) 
{
    ...
    *size = nRows; 
    ...
    #ifdef USE_HP
   	*val_ptr =  (DTYPE *)sds_alloc_non_cacheable(nElements * sizeof(DTYPE));
   	*cols_ptr =(int *)sds_alloc_non_cacheable(nElements * sizeof(int));
   	*rowDelimiters_ptr = (int *)sds_alloc_non_cacheable((nRows+1) * sizeof(int));
    #else
