...
float aproxGPUTh_multi(int chunk) {
    int measurements=lastmeasurements;
    int intervalo = 0;
    bool found = false;
    if (calcluateC1C2) {
...
void getLastChunks_multi(int remain, int* chCPU, int *chGPU, int nCpus,int nGpus) {
    ...
    float tgpu = (float) remain / (aproxGPUTh(remain/nGPus)*nGpus); // se calculan C1 y C2
    if (tgpu < tmin) {
        //(*chCPU) = 0;
        //(*chGPU) = remain;
        int align = ALIGNFLOORGPU(remain/nGpus);
        if (align > 0) {  // solo si se le puede dar alineado
            (*chGPU) = align;
            (*chCPU) = (remain - align*nGpus) / nCpus;
            tmin = tgpu;
        }
    }
...
#endif
// transformación imcompleta a partir de aquí. (ANDRES)
    float thet;
    for (int i = 0; i < lastmeasurements; i++) {
...
public:
...
#ifdef DEBUG
                    if(stopConditionModeOn) cerr << "  stopConditionModeOn " << " lastCPU: " << lastChCPU << " lastFPGA: " <<lastChGPU << endl;
#endif
...
#endif
                    if (lastChGPU > 0 && end-begin> lastChCPU/2) {
                        int auxEnd = begin + lastChGPU;
                        auxEnd = (auxEnd > end) ? end : auxEnd;
                        auxEnd = (end-auxEnd < 4) ? end : auxEnd; // arrastrar el último resto
                        bundle->begin = begin;
                        bundle->end = auxEnd;
                        begin = auxEnd;
                        meanchunkGPU+=(float)(bundle->end-bundle->begin);
    ...
    bool GPU_Stop_Condition() {
        ...
        if ((nCPUs > 1) && fGvalid && (((chunkGPU / fG)*nCPUs) > (((end - begin) - chunkGPU*nGPUs) / (nCPUs)) ) )
            stopGPU = true;

        if((nCPUs ==0)  && ((chunkGPU*nGPUs) >(end - begin)))   stopGPU = true;


        getLastChunks_multi(end - begin, &lastChCPU, &lastChGPU, nCPUs,nGPUs);
        return stopGPU;
    }
};
