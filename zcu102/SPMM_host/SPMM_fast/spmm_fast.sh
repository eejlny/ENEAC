...
# Date: 15 Apr 2019
# Description: SPMM control script for easier data collection
...
do
     if [[ -z $outmatrix_file ]]; then
          spmm_output+=$(./SPMM $matrix $cpuc $hpacc $hpcacc $chunk $ioctl 2> /dev/null)
     else
          spmm_output+=$(./SPMM $matrix $cpuc $hpacc $hpcacc $chunk $ioctl $outmatrix_file 2> /dev/null)
     fi
done
...
#Rows on CPU
IFS=";" read -a cpu_rows <<< $(echo -e "$spmm_output" | awk -v SEP=' ' 'BEGIN{FS=SEP}{if ($1=="Total" && $2=="n." && $5=="CPU:" ){ print $6 }}' | tr "\n" ";" | head -c -1)
#Rows on FPGA
IFS=";" read -a fpga_rows <<< $(echo -e "$spmm_output" | awk -v SEP=' ' 'BEGIN{FS=SEP}{if ($1=="Total" && $2=="n." && $5=="FPGA:" ){ print $6 }}' | tr "\n" ";" | head -c -1)
#CPU Power
IFS=";" read -a cpu_power <<< $(echo -e "$spmm_output" | awk -v SEP=' ' 'BEGIN{FS=SEP}{if ($1=="Full" && $2=="Power" && $4=="POWER" ){ print $6 }}' | tr "\n" ";" | head -c -1)
#FPGA Power    
IFS=";" read -a fpga_power <<< $(echo -e "$spmm_output" | awk -v SEP=' ' 'BEGIN{FS=SEP}{if ($1=="Programmable" && $2=="Logic" && $4=="POWER" ){ print $6 }}' | tr "\n" ";" | head -c -1)
#CPU Energy
IFS=";" read -a cpu_energy <<< $(echo -e "$spmm_output" | awk -v SEP=' ' 'BEGIN{FS=SEP}{if ($1=="Full" && $2=="Power" && $4=="ENERGY" ){ print $6 }}' | tr "\n" ";" | head -c -1)
#FPGA Energy
