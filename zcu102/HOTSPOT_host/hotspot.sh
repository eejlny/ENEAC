...
do
    case $opt in
          h)
               ...
               echo "-f [INT] -> Select square buffer dimention (up to and including 6500)."
               echo "-a [FILE] -> Select the temperature input file."
               echo "-b [FILE] -> Select the power input file."
               echo "-e -> Enable continuous read. Default is ON."
               ...
               echo "Mandatory options are: -n [INT] -a [FILE] -b [FILE] -c [0-4] -p [0-4] -l [0-2] -k [INT]"
               exit 0 
               ;;
...
if [[ -z $bufs ]]; then
     ...
     if ! [[ $bufs =~ ^[0-9]+$ && $bufs -ge 1 && $bufs -le 6500 ]]; then
          echo "Invalid input: specified value for number of buffer size -f $bufs needs to be a positive integer number between 1 and 6500!" >&2
          exit 1
     fi
fi
...
if [[ -z $contin ]]; then
    	contin=1
     echo "Using default value for continuous reading -> $contin (keep rereading input files intill buffer fills)!"     
fi
...
do
     if [[ -z $out_file ]]; then
          hotspot_output+=$(./HOTSPOT $bufs $temps $powers $contin $cpuc $hpacc $hpcacc $chunk $ioctl 2> /dev/null)
          #echo "./HOTSPOT $bufs $temps $powers $contin $cpuc $hpacc $hpcacc $chunk $ioctl 2> /dev/null"
          #exit
     else
          hotspot_output+=$(./HOTSPOT $bufs $temps $powers $contin $cpuc $hpacc $hpcacc $chunk $ioctl $out_file 2> /dev/null)
     fi
done
