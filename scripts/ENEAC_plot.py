...
# Date: 18 Apr 2019
# Description:  Plotting script for ENEAC HOTSPOT using matplotlip
...
def main(argv):
        ...
        if opt == '-h':
            ...
            try:
                ...
                if arg in inputfile:
                    ...
                    if fileopentest.readline() != 'Run[#]\tCPU Cores[#]\tFPGA HP Accelerators[#]\tFPGA HPC Accelerators[#]\tChunk Size[#]\tIOCTL Enabled\tExecution Time[ms]\tWorkload on CPU[#]\tWorkload on FPGA[#]\tScheduler Type\tCPU Power[W]\tFPGA Power[W]\tCPU Energy[J]\tFPGA Energy[J]\n':
                        ...
                        print 'Run[#]\tCPU Cores[#]\tFPGA HP Accelerators[#]\tFPGA HPC Accelerators[#]\tChunk Size[#]\tIOCTL Enabled\tExecution Time[ms]\tWorkload on CPU[#]\tWorkload on FPGA[#]\tScheduler Type\tCPU Power[W]\tFPGA Power[W]\tCPU Energy[J]\tFPGA Energy[J]'
    ...
    if plottype == 2:   
        ...
        for num, conf in enumerate(configs):
            ...
            if ideal_multiplier > 0:
                bar_width = ((bins_np[1] - bins_np[0])/(len(configs)-1))*0.75
                bar_offset = (bar_width*(len(configs)-1))/2
            else:
            ...
            if ideal_multiplier > 0 and num == len(configs)-1:
                templine = mlines.Line2D([i[0] for i in temparr], np.array([i[1] for i in temparr]) + np.array([i[2] for i in temparr]), label=configs[num], color = c)
                ax1.add_line(templine)
            else:
                tempbar = ax1.bar(bins_np-bar_offset+num*bar_width,[i[1] for i in temparr], bar_width, alpha = opacity_front, edgecolor='black', color=c, align = 'edge', label=configs[num])
                ...
                ax1.errorbar(bins_np-bar_offset+num*bar_width+bar_width/2, [i[1] for i in temparr], [i[2] for i in temparr], lolims='False', fmt='none' ,ecolor=tempbar.patches[0].get_facecolor())
        ...
        ax1.set_ybound(upper=(max(np.array([i[1] for i in temparr]) + np.array([i[2] for i in temparr]))+max(np.array([i[1] for i in temparr]) + np.array([i[2] for i in temparr]))*0.05))
        #ax2.set_yticks([])
        #ax2.set_yticklabels([])
        ...
        ax1.legend(handles=ax1handles,loc=0,handler_map={mpatches.Arrow : HandlerPatch(patch_func=make_legend_arrow)})
    ...
    if plottype == 3:
        ...
        opacity_back = 0.75
        c=next(color)
        tempbar = ax1.bar(index+bar_width, cpu_power_np+fpga_power_np, bar_width, alpha=opacity_front, color=c, edgecolor='black', label='Power')
    ...
    if plottype == 4:   
        ...
        for num, infile in enumerate(inputfile):
            ...
            if temp_conf not in configs:
                configs.append(temp_conf)
                conf_throughput.append([])
                conf_throughput[-1].append((cpu_rows[num]+fpga_rows[num])/exec_time[num]) #add value to last element in array
                conf_chunk_size.append([])
                conf_chunk_size[-1].append(chunk_size[num])
            #If configuration already found just add contents     
            else:
                temp_ind=configs.index(temp_conf) #get index of entry
                conf_throughput[temp_ind].append((cpu_rows[num]+fpga_rows[num])/exec_time[num])
                conf_chunk_size[temp_ind].append(chunk_size[num])                               
        ...
        if ideal_multiplier > 0:
             for num, infile in enumerate(ideal_inputfile):
                  ...
                  if temp_conf not in configs:
                       configs.append(temp_conf)
                       conf_throughput.append([])
                       conf_throughput[-1].append((ideal_cpu_rows[num]+ideal_fpga_rows[num])/ideal_exec_time[num]) #add value to last element in array
                       conf_chunk_size.append([])
                       conf_chunk_size[-1].append(ideal_chunk_size[num])
                  #If configuration already found just add contents     
                  else:
                       temp_ind=configs.index(temp_conf) #get index of entry
                       conf_throughput[temp_ind].append((ideal_cpu_rows[num]+ideal_fpga_rows[num])/ideal_exec_time[num])
                       conf_chunk_size[temp_ind].append(ideal_chunk_size[num])
        ...
        if multihap_inputfile != []:
            for num, infile in enumerate(multihap_inputfile):
                  ...
                  if temp_conf not in configs:
                       configs.append(temp_conf)
                       conf_throughput.append([])
                       conf_throughput[-1].append((multihap_cpu_rows[num]+multihap_fpga_rows[num])/multihap_exec_time[num]) #add value to last element in array
                       conf_chunk_size.append([])
                       conf_chunk_size[-1].append(multihap_chunk_size[num])
                  #If configuration already found just add contents     
                  else:
                       temp_ind=configs.index(temp_conf) #get index of entry
                       conf_throughput[temp_ind].append((multihap_cpu_rows[num]+multihap_fpga_rows[num])/multihap_exec_time[num])
                       conf_chunk_size[temp_ind].append(multihap_chunk_size[num])                          
        ...
        for num, conf in enumerate(configs):
            temparr = []
            for i in range(len(conf_chunk_size[num])):
                temparr.append([conf_chunk_size[num][i],conf_throughput[num][i]])
            temparr = sorted(temparr, key=itemgetter(0))
            marker_y = max([i[1] for i in temparr])
            marker_index = [i[1] for i in temparr].index(marker_y)
            marker_x = [i[0] for i in temparr][marker_index]
            #templine = ax.plot([i[0] for i in temparr],[i[1] for i in temparr], label=configs[num])
            c=next(color)
    ...
    if plottype == 5:   
        ...
        for num, infile in enumerate(inputfile):
            ...
            if temp_conf not in configs:
                configs.append(temp_conf)
                conf_throughput.append([])
                conf_throughput[-1].append((cpu_rows[num]+fpga_rows[num])/(cpu_energy[num]+fpga_energy[num])) #add value to last element in array
                conf_chunk_size.append([])
                conf_chunk_size[-1].append(chunk_size[num])
            #If configuration already found just add contents     
            else:
                temp_ind=configs.index(temp_conf) #get index of entry
                conf_throughput[temp_ind].append((cpu_rows[num]+fpga_rows[num])/(cpu_energy[num]+fpga_energy[num]))
                conf_chunk_size[temp_ind].append(chunk_size[num])                               
        ...
        if ideal_multiplier > 0:
             for num, infile in enumerate(ideal_inputfile):
                  ...
                  if temp_conf not in configs:
                       configs.append(temp_conf)
                       conf_throughput.append([])
                       conf_throughput[-1].append((ideal_cpu_rows[num]+ideal_fpga_rows[num])/(ideal_cpu_power[num]+ideal_fpga_power[num])) #add value to last element in array
                       conf_chunk_size.append([])
                       conf_chunk_size[-1].append(ideal_chunk_size[num])
                  #If configuration already found just add contents     
                  else:
                       temp_ind=configs.index(temp_conf) #get index of entry
                       conf_throughput[temp_ind].append((ideal_cpu_rows[num]+ideal_fpga_rows[num])/(ideal_cpu_power[num]+ideal_fpga_power[num]))
                       conf_chunk_size[temp_ind].append(ideal_chunk_size[num])
        ...
        if multihap_inputfile != []:
            for num, infile in enumerate(multihap_inputfile):
                  ...
                  if temp_conf not in configs:
                       configs.append(temp_conf)
                       conf_throughput.append([])
                       conf_throughput[-1].append((multihap_cpu_rows[num]+multihap_fpga_rows[num])/(multihap_cpu_power[num]+multihap_fpga_power[num])) #add value to last element in array
                       conf_chunk_size.append([])
                       conf_chunk_size[-1].append(multihap_chunk_size[num])
                  #If configuration already found just add contents     
                  else:
                       temp_ind=configs.index(temp_conf) #get index of entry
                       conf_throughput[temp_ind].append((multihap_cpu_rows[num]+multihap_fpga_rows[num])/(multihap_cpu_power[num]+multihap_fpga_power[num]))
                       conf_chunk_size[temp_ind].append(multihap_chunk_size[num])                          
        ...
        #ax.set(xlabel='Chunk Size [#]', ylabel='Execution Time [ms]')
        ax.set_xlabel('Chunk Size [#]', fontweight='bold')
        ax.set_ylabel('Throughput [Workload/Joule]', fontweight='bold')
